...
# Load the saved state dictionary into the instantiated model 'm'
# map_location=device ensures it loads correctly whether on CPU or GPU/MPS
print(f"Loading model weights from {model_path}...")
m.load_state_dict(torch.load(model_path, map_location=device))

# Set the model to evaluation mode
# This is crucial! It disables dropout and ensures consistent behavior during inference.
m.eval() 

print("Model loaded successfully. Generating text...")

# Infer from the loaded model
context = torch.zeros((1, 1), dtype=torch.long, device=device) # Starting context for generation
